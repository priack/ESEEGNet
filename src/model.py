import keras
from keras.layers import Conv2D, Dense, Dropout, BatchNormalization, Activation, Concatenate, DepthwiseConv2D, Flatten, Reshape
from keras import Model, Input

def convolution_block(x, nFilters: int, size: tuple[int, int], stride: int,  dropRate: float, name: str):
    x = Conv2D(nFilters, size, (1, stride), name=name)(x)
...
def depth_convolution_block(x, length: int, stride: int, dropRate: float, name: str):
    x = DepthwiseConv2D((1, length), (1, stride), use_bias=False, name=name)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Dropout(dropRate)(x)
    return x
...
def time_convolution_stack(x, length: int, nLayers: int, dropRate: float, name: str):
...
def ESEEGNet(shape: tuple) -> Model:
    ...
    inputs = Reshape((d1 * d2, t, 1))(inputs)
    ...
    # Shape [Batch, d1, d2, nTimeFilters * 3]
    x = Reshape((d1, d2, nTimeFilters * 3))(x)
